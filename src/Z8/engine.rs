...
mod phase_poly_tests {
    ...
    fn test_clifford_plus_s_circuit() {
        ...
        let circuit = QuantumCircuit {
            ...
            gates: vec![
                ...
                Gate { name: "s".to_string(), qubits: vec![0], params: vec![] },
    ...
    fn test_clifford_plus_t_relative_phase() {
        ...
        let circuit = QuantumCircuit {
            ...
            gates: vec![
                Gate { name: "h".to_string(), qubits: vec![0], params: vec![] },
                Gate { name: "t".to_string(), qubits: vec![0], params: vec![] },
                Gate { name: "h".to_string(), qubits: vec![1], params: vec![] },
                Gate { name: "cz".to_string(), qubits: vec![0, 1], params: vec![] },
                Gate { name: "h".to_string(), qubits: vec![1], params: vec![] },
            ],
	...
	for gate in &circuit.gates {
		...
		match name.as_str() {
			...
			"cz" => {
				let q0 = qubits[0];
				let q1 = qubits[1];
				terms.push((4, vec![*wire_array[q0].last().unwrap(), *wire_array[q1].last().unwrap()]));
			},
			"s" => {
				let q = qubits[0];
