...
class DicksonEngine:
    ...
    def _plan_dickson(self, b: np.ndarray, n: int):
        ...
        while p + 1 < n:
            pivot = next(
                ((i, j) for i in range(p, n) for j in range(i + 1, n) if b_w[i, j] == 1),
                None,
            )
            if not pivot:
                break
    ...
    def _eval_canonical_sum(self, vu: np.ndarray, nu: int) -> complex:
        ...
        while p < self.rank:
            ...
            for x1, x2 in [(0, 0), (0, 1), (1, 0), (1, 1)]:
                ph = (
                    (2 if self.b_reduced[p, p + 1] and x1 and x2 else 0)
                    + (vu[p] if x1 else 0)
                    + (vu[p + 1] if x2 else 0)
                ) % 4
                p_s += phases[ph]
...
class UniversalQC:
    ...
    def build_tree(self):
        ...
        l_h = [next((i for i in range(n - 1, -1, -1) if data[i].operation.name.lower() == 'h' and self.circuit.find_bit(data[i].qubits[0]).index == q), None) for q in range(self.num_qubits)]
    ...
    def get_statevector(self, x: int = 0) -> np.ndarray:
        ...
        for q in range(self.num_qubits):
            if self.output_phases[q]:
                mask = (idx_arr >> q) & 1
                final_sv[mask == 1] *= np.exp(1j * self.output_phases[q])
        return final_sv
